...
static Ast *ast_lvar(Ctype *ctype, char *vname) {
  Ast *r = malloc(sizeof(Ast));
  r->type = AST_LVAR;
  r->ctype = ctype;
  r->lname = vname;
...
static Ast *ast_func(Ctype *rettype, char *fname, List *params, List *body,
                     List *locals) {
  Ast *r = malloc(sizeof(Ast));
  r->type = AST_FUNC;
  r->ctype = rettype;
...
static Ctype *result_type(char op, Ctype *a, Ctype *b) {
  jmp_buf jmpbuf;
  if (setjmp(jmpbuf) == 0) return result_type_int(&jmpbuf, op, a, b);
...
static Ctype *convert_array(Ast *ast) {
  if (ast->ctype->type != CTYPE_ARRAY) return ast->ctype;
...
static Ast *read_expr(int prec) {
  ...
  for (;;) {
    ...
    if (!is_punct(tok, '=') && asttype->type != CTYPE_PTR &&
...
static bool is_type_keyword(Token *tok) { return get_ctype(tok) != NULL; }
...
struct Ast *read_decl(void) {
  ...
  Token *tok = read_token();
  if (is_punct(tok, '=')) return read_decl_array_init(var);
  unget_token(tok);
  expect(';');
...
static Ast *read_for_stmt(void) {
  ...
  Ast *step = is_punct(peek_token(), ')') ? NULL : read_expr(0);
  expect(')');
  expect('{');
...
static void ast_to_string_int(Ast *ast, String *buf) {
  ...
  switch (ast->type) {
    ...
    case AST_IF:
      ...
      if (ast->els) string_appendf(buf, " %s", block_to_string(ast->els));
      string_append(buf, ')');
      break;
    case AST_FOR:
      string_appendf(buf, "(for %s %s %s ", ast_to_string(ast->forinit),
                     ast_to_string(ast->forcond), ast_to_string(ast->forstep));
