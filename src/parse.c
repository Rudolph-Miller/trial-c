...
static Ast *ast_float(float fval) {
  Ast *r = malloc(sizeof(Ast));
  r->type = AST_LITERAL;
  r->ctype = ctype_float;
  r->fval = fval;
...
static Ast *ast_func(Ctype *rettype, char *fname, List *params, Ast *body,
                     List *localvars) {
  Ast *r = malloc(sizeof(Ast));
  r->type = AST_FUNC;
  r->ctype = rettype;
...
static Ast *find_var(char *name) {
  for (Env *p = localenv; p; p = p->next) {
    for (Iter *i = list_iter(p->vars); !iter_end(i);) {
      Ast *v = iter_next(i);
      if (!strcmp(name, v->varname)) return v;
    }
  }
  return NULL;
}
...
static bool is_right_assoc(Token *tok) { return is_punct(tok, '='); }
...
static Ast *read_ident_or_func(char *name) {
  Token *tok = read_token();
  if (is_punct(tok, '(')) return read_func_args(name);
  unget_token(tok);
  Ast *v = find_var(name);
...
static Ast *read_prim(void) {
  ...
  switch (tok->type) {
    ...
    case TTYPE_NUMBER:
      if (is_int(tok->sval)) return ast_int(atoi(tok->sval));
      if (is_float(tok->sval)) return ast_float(atof(tok->sval));
...
static Ctype *result_type_int(jmp_buf *jmpbuf, char op, Ctype *a, Ctype *b) {
  ...
  switch (a->type) {
    ...
    case CTYPE_FLOAT:
      if (b->type == CTYPE_FLOAT) return ctype_float;
      goto err;
    case CTYPE_ARRAY:
...
static Ast *read_unary_expr(void) {
  ...
  if (is_punct(tok, '*')) {
    ...
    Ast *r = ast_uop(AST_DEREF, operand->ctype->ptr, operand);
    return r;
  }
  if (is_punct(tok, '!')) {
    Ast *operand = read_unary_expr();
...
static Ast *read_expr_int(int prec) {
  ...
  for (;;) {
    ...
    if (!rest) error("Second operator missing");
    ast = ast_binop(tok->punct, ast, rest);
  }
}

static Ast *read_expr(void) { return read_expr_int(MAX_OP_PRIO); }

static Ctype *get_ctype(Token *tok) {
  if (!tok) return NULL;
  if (tok->type != TTYPE_IDENT) return NULL;
...
static char *read_struct_union_tag(void) {
  Token *tok = read_token();
  if (tok->type == TTYPE_IDENT) return tok->sval;
  unget_token(tok);
  return NULL;
}
...
static List *read_struct_union_fields(void) {
  ...
  expect('{');
  for (;;) {
    if (!is_type_keyword(peek_token())) break;
    Token *name;
...
static Ctype *read_decl_spec(void) {
  Token *tok = read_token();
  Ctype *ctype =
      is_ident(tok, "struct") ? read_struct_def() : is_ident(tok, "union")
                                                        ? read_union_def()
                                                        : get_ctype(tok);
  if (!ctype) error("type expected, but got %s", token_to_string(tok));
  for (;;) {
    tok = read_token();
    if (!is_punct(tok, '*')) {
      unget_token(tok);
...
static Ast *read_decl_init_val(Ast *var) {
  if (var->ctype->type == CTYPE_ARRAY) {
    ...
    if (var->ctype->len == -1) {
      ...
      error("Invalit array initializer: expected %d items, but got %d",
            var->ctype->len, len);
    }
    expect(';');
  ...
  Ast *init = read_expr();
  expect(';');
  if (var->type == AST_GVAR) check_intexp(init);
  return ast_decl(var, init);
}
...
static Ctype *read_array_dimensions_int(Ctype *basetype) {
  ...
  if (sub) {
    if (sub->size == -1 && dim == -1) error("Array size is not specified");
...
static Ast *read_decl_init(Ast *var) {
  Token *tok = read_token();
  if (is_punct(tok, '=')) return read_decl_init_val(var);
  if (var->ctype->len == -1) error("Missing array initializer");
  unget_token(tok);
  expect(';');
  return ast_decl(var, NULL);
}

struct Ctype *read_decl_int(Token **name) {
...
static List *read_params(void) {
  ...
  for (;;) {
    ...
    if (ctype->type == CTYPE_ARRAY) ctype = make_ptr_type(ctype->ptr);
    list_push(params, ast_lvar(ctype, pname->sval));
    Token *tok = read_token();
    if (is_punct(tok, ')')) return params;
    if (!is_punct(tok, ','))
      error("Comman expecetd, but got %s", token_to_string(tok));
  }
}
...
static Ast *read_decl_or_func_def(void) {
  ...
  if (is_punct(tok, '(')) return read_func_def(ctype, name->sval);
  if (is_punct(tok, ';')) {
    read_token();
    Ast *var = ast_gvar(ctype, name->sval, false);
    return ast_decl(var, NULL);
