...
struct Ast *read_decl(void) {
  ...
  if (tok->type != TTYPE_IDENT)
    error("Identifer expected, but got %s", token_to_string(tok));
  Token *varname = tok;
  for (;;) {
    tok = read_token();
    if (is_punct(tok, '[')) {
...
static void emit_assign(Ast *var, Ast *value) {
  ...
  switch (var->type) {
    case AST_LVAR:
      ...
      break;
    case AST_LREF:
      emit_lsave(var->lref->ctype, var->lref->loff, var->loff);
      break;
    case AST_GVAR:
      emit_gsave(var, 0);
      break;
...
void emit_binop(Ast *ast) {
  ...
  switch (ast->type) {
    ...
    case '/':
      break;
    default:
      error("Invalid operation '%c'", ast->type);
  }
  emit_expr(ast->left);
  printf("push %%rax\n\t");
  emit_expr(ast->right);
