...
static void emit_gload(Ctype *ctype, char *label, int off) {
  ...
  if (ctype->size == 1) emit("mov $0, %%eax");
  if (off)
...
static void emit_toint(Ctype *ctype) {
  SAVE;
  if (ctype->type != CTYPE_FLOAT) return;
  emit("cttss2si %%xmm0, %%eax");
}

static void emit_tofloat(Ctype *ctype) {
  SAVE;
  if (ctype->type != CTYPE_INT) return;
...
static void emit_binop_int_arith(Ast *ast) {
  ...
  switch (ast->type) {
    ...
    case '/':
      break;
    default:
      error("Invalid operaton: '%c'", ast->type);
  }
  emit_expr(ast->left);
  emit_toint(ast->left->ctype);
  emit("push %%rax");
...
static void emit_load_deref(Ctype *result_type, Ctype *operand_type, int off) {
  ...
  if (result_type->size == 1) emit("mov $0, %%ecx");
  if (off)
...
static void emit_data(Ast *v) {
  ...
  assert(v->declinit->type == AST_LITERAL &&
         v->declinit->ctype->type == CTYPE_INT);
...
static void emit_func_prologue(Ast *func) {
  ...
  if (off) emit("sub $%d, %%rsp", align(-off, 16));
}
